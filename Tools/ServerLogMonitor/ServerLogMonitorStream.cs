...
internal sealed partial class ServerLogMonitorTool
{
    ...
    private void DrawStreamFilters()
    {
        ...
        GUI.enabled = controlsEnabled && !string.IsNullOrEmpty(_streamSearch);
        ...
        GUI.enabled = controlsEnabled && (_filteredStream.Count > 0 || _entries.Count > 0);
        ...
        GUI.enabled = controlsEnabled && _selectedEntries.Count > 0;
        if (GUILayout.Button(
                new GUIContent(
                    "Copy selected",
        ...
        bool includeMetadata = ProfilerGui.ToggleLayout(
            _theme,
            _app.Config.ServerLogCopyMetadata.Value,
            new GUIContent(
                "Copy metadata",
            ...
            125f,
            _labelStyle);
        if (includeMetadata != _app.Config.ServerLogCopyMetadata.Value)
            _app.Config.ServerLogCopyMetadata.Value = includeMetadata;
        GUILayout.EndHorizontal();
        GUILayout.Space(2f);
    }
    ...
    private void DrawHistoryControls()
    {
        ...
        bool loading = _historyRequestPending;
        bool oldEnabled = GUI.enabled;
        GUI.enabled = oldEnabled && _subscribed && !loading && _historyHasMore;
        if (GUILayout.Button(loading ? "Loading..." : "Load older", GUILayout.Width(88f)))
            RequestOlderHistory();
        GUI.enabled = oldEnabled;

        GUI.enabled = oldEnabled && _loadedHistoryEntries > 0 && !loading;
    ...
    private void CopyFilteredStream()
    {
        ...
        for (int i = 0; i < _filteredStream.Count; i++)
        {
            ...
            {
                builder.AppendLine();
                if (_app.Config.ServerLogCopyMetadata.Value)
                    builder.AppendLine();
            }
            builder.Append(_filteredStream[i].GetClipboardText(_app.Config.ServerLogCopyMetadata.Value));
        }
    ...
    private void CopySelectedStream()
    {
        ...
        for (int i = 0; i < _filteredStream.Count; i++)
        {
            ...
            builder.Append(entry.GetClipboardText(_app.Config.ServerLogCopyMetadata.Value));
        }
    ...
    private void DrawStreamHeader()
    {
        ...
        float x = rect.x;
        DrawHeaderCell(ref x, rect.y, timeWidth, rect.height, "Time", "Server event time converted to the client local time zone. Unity timestamps are removed from Message and kept here.");
        DrawHeaderCell(ref x, rect.y, levelWidth, rect.height, "Level", "BepInEx log level.");
        DrawHeaderCell(ref x, rect.y, sourceWidth, rect.height, "Source", "Dedicated-server BepInEx logger source. Unity messages are normally forwarded through a Unity Log source.");
    ...
    private void DrawStreamDetails(float height)
    {
        ...
        if (GUILayout.Button(_selectedEntries.Count > 1 ? "Copy selected" : "Copy", GUILayout.Width(_selectedEntries.Count > 1 ? 100f : 58f)))
        {
            if (_selectedEntries.Count > 1)
                ...
                GUIUtility.systemCopyBuffer = entry.GetClipboardText(_app.Config.ServerLogCopyMetadata.Value);
        }
