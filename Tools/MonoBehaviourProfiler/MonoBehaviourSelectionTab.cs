...
internal sealed partial class MonoBehaviourProfilerTool
{
    private void DrawSelectionTab()
    {
        if (!_listReady)
        {
            ...
            Label("MonoBehaviour list is not loaded.", GUILayout.ExpandWidth(true));
    ...
    private void DrawSelectionBulkActions()
    {
        ...
        if (GUILayout.Button(new GUIContent(
                "Refresh list",
                "Rescan loaded assemblies and rebuild the callback list.\nUnsaved selection changes are discarded."),
        ...
        if (GUILayout.Button(new GUIContent(
                "Enable all",
                "Enable every callback visible through the current Source, Present in active scene and Search filters.\nCallbacks hidden by the current filters are not changed."),
            GUILayout.Width(88f)))
        {
        ...
        if (GUILayout.Button(new GUIContent(
                "Enable default callbacks",
                "Enable the default frame callbacks added by mods: Update, FixedUpdate, LateUpdate and OnGUI.\nExisting manually enabled callbacks remain selected."),
        ...
        if (GUILayout.Button(new GUIContent(
                "Disable all",
                "Clear every selected callback and collapse the selection tree."),
            GUILayout.Width(88f)))
        {
            lock (_lock)
            {
        ...
        if (GUILayout.Button(new GUIContent(
                "Reset to defaults",
                "Restore the default selection: mod frame callbacks enabled, Valheim and Other callbacks disabled."),
        ...
        if (GUILayout.Button(new GUIContent(
                "Apply selection",
                "Apply the current callback selection.\nIf profiling is active, instrumentation is restarted and statistics are reset."),
            buttonStyle,
    ...
    private void DrawSelectionSourceRow()
    {
        GUILayout.BeginHorizontal();
        ...
        GUILayout.Label(
            new GUIContent(
                "Source:",
                "Choose one source category to display. The tabs only filter the view and do not change saved selection."),
            _labelStyle,
            GUILayout.Width(52f));
        ...
        BehaviourSource newSource = (BehaviourSource)GUILayout.Toolbar(
            ...
            new[]
            {
                new GUIContent("Mods", "Show MonoBehaviour frame callbacks discovered in BepInEx plugin assemblies."),
                new GUIContent("Valheim", "Show MonoBehaviour frame callbacks declared by Valheim game assemblies."),
                new GUIContent("Other", "Show callbacks from managed assemblies that are neither Valheim nor identifiable BepInEx plugins.")
            },
    ...
    {
        if (!_selectionRowsDirty &&
            string.Equals(_lastSelectionSearch, _search, StringComparison.Ordinal))
            return;
    ...
    private static List<BehaviourMethodEntry> GetVisibleMethods(
    ...
    private void DrawSelectionRows(Rect contentRect, float rowHeight)
    {
        ...
        float visibleBottom = _selectionScroll.y + Mathf.Max(100f, MainWindowHeight - 230f) + VirtualizationOverscan;
        ...
        for (int i = 0; i < _selectionRows.Count; i++)
        {
            ...
            switch (row.Kind)
            {
                ...
                case SelectionRowKind.Type:
                {
                    ...
                    int selected = type?.Methods.Count(method => method.Selected) ?? 0;
                    int total = type?.Methods.Count ?? 0;
                    string text = $"{row.Text}  ({selected}/{total})";
                    toggleRequested |= GUI.Button(
                        new Rect(x, rowY, contentRect.width - x + contentRect.x, rowHeight),
                        text,
                        _headerLabelStyle);
                ...
                case SelectionRowKind.Method:
                {
                    ...
                    bool selected = ProfilerGui.Toggle(
                        ...
                        toggleRect,
                        method.Selected,
                        new GUIContent(method.DisplayName),
                        _labelStyle,
                        0f);
